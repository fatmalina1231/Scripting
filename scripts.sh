...
case ${OPTION} in
	...
	#x) gnuplot -p GraphicScript.plt < /home/Fatma/Desktop/top-output.txt ;;
	x) gnuplot -e "set terminal pngcairo enhanced font 'arial,10' fontscale 1.0 size 600, 400; set output '/home/Fatma/Desktop/out.png'; set style data histogram; set style fill solid border -1; plot for [i=9:10] '/dev/stdin' using i:xtic(12) title col" < ${2} ;;
	#sauvegarde le graph dans un fichier png
	#y) gnuplot -p RTGraph.gnu < /home/Fatma/Desktop/top-output.txt	;;
